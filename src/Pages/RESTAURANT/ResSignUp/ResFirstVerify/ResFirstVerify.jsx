...
import { resEmailVerify, restuSignUpVerify } from '../../../../redux/actions/restaurant';
import Loader from '../../../../Components/Loaders/Loader';

const ResFirstVerify = ({isRestuAuther, isResLoading=true}) => {

  const navigator = useNavigate()
  const dispatch = useDispatch()

  const success = useSelector((state)=> state?.restu?.success)
  const isEmailVerify = useSelector((state)=> state?.restu?.restu?.resEmail?.isVerify)
  const resEmail = useSelector((state)=> state?.restu?.restu?.resEmail?.email)
                ...
                <div className="d-flex flex-column justify-content-center align-items-center h-100 me-xl-4 me-lg-4 me-md-4 me-sm-0 me-0">
                  ...
                  <div className="text-center">
                    ...
                    <div className='w-100 d-flex flex-column align-items-center mt-3'>
                        ...
                        <div className="text-white dont-have-account-text mt-3">hvae an account yet? <NavLink to="/res/login" className='fw-bold'>Login</NavLink> </div>
                    </div>
                  </div>
                </div>
              </div>
