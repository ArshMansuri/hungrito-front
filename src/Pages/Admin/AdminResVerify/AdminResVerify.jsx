...
const AdminResVerify = () => {
  ...
  const acceptResHendler = async()=>{
    try {
        if (resId) {
            setIsLoading(true)
            const {data} = await axios.get(`${BASE_URL}/api/v1/admin/new/restu/accept/${resId}`, {withCredentials: true})
            console.log(data)
  ...
  const rejectResHendler = async()=>{
    try {
        if (resId) {
            setIsLoading(true)
            const {data} = await axios.delete(`${BASE_URL}/api/v1/admin/new/restu/reject/${resId}`, {withCredentials: true})
            console.log(data)
            if(data !== undefined && data?.success === true){
                // show toast
                return navigate(-1)
    ...
    <div className="admin-res-verify-page">
        {
            ...
            <div className="row">
              <div className="col">
                ...
                <div>
                  <strong>Restaurant City:</strong>  {restu?.resCompletAddress?.city || ""}
                </div>
              </div>
            </div>
            <div className=" d-flex justify-content-start mt-3"> 
            <div className="d-grid gap-2 w-50">
                <button className="btn text-white border-0 accept-btn" type="button" style={{backgroundColor: "#ff6600"}} onClick={acceptResHendler}>Accept</button>
                <button className="btn btn-danger border-0" type="button" onClick={rejectResHendler}>Reject</button>
              </div>
              </div>
          </div>
        }
