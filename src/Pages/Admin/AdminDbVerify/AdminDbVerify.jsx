    ...
    useEffect(() => {
      ...
      const acceptDbHendler = async()=>{
        try {
            if (dbId) {
                setIsLoading(true)
                const {data} = await axios.get(`${BASE_URL}/api/v1/admin/new/db/accept/${dbId}`, {withCredentials: true})
                console.log(data)
      ...
      const rejectDbHendler = async()=>{
        try {
            if (dbId) {
                setIsLoading(true)
                const {data} = await axios.delete(`${BASE_URL}/api/v1/admin/new/db/reject/${dbId}`, {withCredentials: true})
                console.log(data)
                if(data !== undefined && data?.success === true){
                    // show toast
                    return navigate(-1)
  ...
  return (
    <div className="admin-db-verify-page">
    {
        ...
        <div className="d-grid gap-2 w-50">
            <button className="btn text-white border-0 accept-btn" type="button" style={{backgroundColor: "#ff6600"}} onClick={acceptDbHendler}>Accept</button>
            <button className="btn btn-danger border-0" type="button" onClick={rejectDbHendler}>Reject</button>
          </div>
          </div>
      </div>
    }
